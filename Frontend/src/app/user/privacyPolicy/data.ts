export const onlinePrivacy = [
    {
      heading: "Online Privacy Policy for Injazrent.ae",
      para:
        "This Privacy Policy describes how Injazrent.ae collects, uses, and protects information obtained through its website (“injazrent.ae”). This Policy applies solely to activities conducted on the Injazrent.ae website and does not extend to any offline or unrelated activities.",
    },
  ];
  
  export const collection = [
    {
      heading: "Anonymous Data Collection",
      para:
        "Injazrent.ae may collect certain anonymous data about your visit to the website. This information, which does not personally identify you, is gathered to improve the website's performance and user experience. Examples of anonymous data collected include, but are not limited to:",
      points: [
        "The type of browser you are using",
        "The duration of your visit to the website",
      ],
      para2:
        "This data is used for analytical purposes to enhance our services and optimize website functionality.",
    },
  ];
  
  export const idInfo = [
    {
      heading: "Personally Identifiable Information",
      para:
        "While using the Injazrent.ae website, you may be asked to voluntarily provide personally identifiable information.",
      title1: "This information may include:",
      title2: "This data may be collected in situations such as:",
      points1: [
        "The type of Your name",
        "The duration of Your address",
        "The duration of Your telephone number",
        "The duration of Your email address",
      ],
      points2: [
        "The duration of Submitting feedback or inquiries via email",
        "The duration of Registering for services on the website",
        "The duration of Making purchases through the website",
      ],
      para2:
        "You have the option to provide or withhold personally identifiable information. However, certain features or services of the website may be unavailable if you choose not to provide the requested information.",
    },
  ];
  
  export const disclosureInfo = [
    {
      title: "Additional Recommendations for Enhancements:",
      details: [
        "Purpose of Data Collection: Include a section explaining the purposes for which personally identifiable information is collected (e.g., processing transactions, responding to inquiries, or improving services).",
        "Third-Party Disclosure: State whether and how data might be shared with third parties (e.g., service providers or partners) and include a statement on data security.",
        "User Rights: Consider mentioning users' rights regarding their personal data, such as access, correction, or deletion.",
        "Data Retention: Explain how long personal data is retained and the criteria for deletion.",
        "Updates to Policy: Include a statement informing users that the policy may be updated periodically and how updates will be communicated.",
        "Contact Information: Add a clear method for users to reach out with questions or concerns regarding the policy.",
        "Would you like assistance drafting specific sections or additional clauses?"
      ],
    },
    {
      title: "Use and Disclosure of Information",
      details: [
        "At Injazrent.ae, we are committed to protecting your privacy. The following outlines how we use and disclose the information collected on our site:",
      ],
    },
    {
      title: "Primary Use of Information",
      details: [
        "We do not sell, trade, or rent your personally identifiable information to others, except as described in this policy.",
        "Information collected through our site is primarily used for:",
        "Processing orders",
        "Updating you on your order status",
        "Notifying you of products, services, or special offers of potential interest",
        "Conducting statistical analyses to improve site functionality and user experience",
      ],
    },
    {
      title: "Disclosure to Third Parties",
      details: [
        "We may disclose your delivery information to third parties as necessary for:",
        "Order tracking",
        "Payment processing (e.g., checks or money orders)",
        "Fulfilling and delivering your order",
        "Enhancing site functionality",
        "Conducting data analyses",
        "Sending promotional emails directly from Injazrent.ae",
        "For example, your mailing address may be provided to the delivery service to ensure the fulfillment of your order.",
      ],
    },
    {
      title: "Credit/Debit Card Information",
      details: [
        "Injazrent.ae ensures that all credit/debit card details and personally identifiable information are not stored, sold, shared, rented, or leased to any third parties.",
      ],
    },
    
    { title: "Cookies and Aggregated Data", details: [
"Our car rental website uses cookies to enhance user experience, remember preferences, and personalize content.",
"We also collect aggregated, non-personal data to analyze website traffic and improve our services.",
    ] },
    
    {
      title: "1.	Cookies:",
      details: [
        "Injazrent.ae utilizes cookies, which are small bits of data stored in your browser, to determine whether you have visited the home page in the past.",
        "No other user-specific information is gathered through cookies.",
      ],
    },
    {
      title: "2.	Aggregated Data:",
      details: [
        "We may use non-personal aggregated data (data that does not identify an individual) to:",
        "Enhance the operation and usability of our website",
        "Analyze user interest in specific website areas",
      ],
    },
    {
      title: "Publishing and Feedback",
      details: [
        "If you provide Injazrent.ae with content for publishing (e.g., feedback or testimonials), we may publish your username or other identifying information with your permission.",
      ],
    },
    
    { 
      title: "Legal Disclosures:", 
      details: [
      "Legal disclosures in car rental services outline the terms and conditions between the customer and the company, including rental duration, insurance coverage, and liability.",
"They clarify responsibilities in case of accidents, damages, or traffic violations during the rental period.",
   ], },

    {
      title: "Compliance with Legal Obligations:",
      details: [
        "Injazrent.ae may disclose personally identifiable information in response to:",
        "A subpoena",
        "A court order",
        "A request from a law enforcement agency",
        "Other legal obligations or requirements",
      ],
    },
    {
      title: "Corporate Transactions:",
      details: [
        "o	In the event of bankruptcy, or during a corporate reorganization such as a merger, acquisition, or transfer of ownership, your personally identifiable information may be provided to the involved parties.",
      ],
    },
    {
      title: "Recommendations for Further Enhancements:",
      details: [
        "Data Security Measures: Include a section on how user data is safeguarded (e.g., encryption, access restrictions).",
        "User Consent: Reinforce that users consent to the described uses and disclosures of their information by using the website.",
        "Contact Information: Provide a clear way for users to contact your company with concerns or inquiries regarding their personal information.",
        "Opt-Out Options: Allow users to opt-out of receiving promotional communications or data tracking.",
      ],
    },
    {
      title: "Security",
      details: [
        "Injazrent.ae takes appropriate measures to ensure the privacy and security of user data through various hardware and software methodologies. However, no method of data transmission or storage over the internet can be guaranteed as 100% secure. As such, Injazrent.ae cannot ensure or warrant the absolute security of any information disclosed online.",
      ],
    },
    {
      title: "Third-Party Websites",
      details: [
        "Injazrent.ae is not responsible for the privacy practices or policies of third-party websites to which it links. Please note:",
        "Different rules regarding the collection and use of your personal information may apply if you interact with third-party websites.",
        "Injazrent.ae strongly recommends that you review the privacy policies of these websites before providing any personal data.",
        "Our site may contain links to third-party sites owned and operated independently. These sites may send their own cookies, collect data, or solicit personally identifiable information. Injazrent.ae has no control over these practices and cannot assume responsibility for how such third parties manage user information.",
        "If you have concerns about the use of your data by third parties, you should contact them directly.",
      ],
    },
    {
      title: "Minors",
      details: [
        "Injazrent.ae does not knowingly collect personal information from minors under the age of 18.",
        "Minors are not permitted to use the Injazrent.ae website or its services.",
        "Injazrent.ae requests that individuals under the age of 18 refrain from submitting any personal information to the website.",
        "Since data from minors is not knowingly collected, Injazrent.ae does not knowingly distribute such information.",
        "If you believe that information regarding a minor under 18 has been inadvertently collected, please contact info@injazrent.ae for assistance with removal or correction.",
      ],
    },
    {
      title: "Modifications to the Privacy Policy",
      details: [
        "Injazrent.ae reserves the right to update or modify its Website Policies and Terms & Conditions to align with evolving requirements and standards.",
        "Customers are encouraged to review these sections regularly to stay informed about any changes.",
        "Modifications will take effect on the day they are posted.",
      ],
    },
    {
      title: "Suggestions for Further Improvement:",
      details: [
        "Effective Date of Policy: Add an Effective Dateto indicate when the policy was last updated.",
        "User Notification of Changes: Specify if users will be notified of changes through email or a website announcement.",
        "Clear Definitions: Define key terms (e.g., “minor,” “third party,” “personal information”) for clarity.",
      ],
    },
  ];